...
## Intuition revision (presentation revision 3)

Each lesson defaults to Intuition, with concrete analogies, a guide to reading its picture, plain-language controls and prediction questions, and three live explanation steps. The Mathematics view exposes the existing symbolic derivation and numerical substitution. Switching language preserves parameters, the selected scene and camera; it stops playback and clears quiz feedback. The choice persists between lessons for the current page session, with no browser storage. Advanced notes remain available in both views.

`calculus-intuition.mjs` is a presentation adapter, not a second engine. Its stories consume the same parameter/result snapshot as the plots and equations. Guided examples apply complete bounded parameter snapshots atomically; manual changes then continue from those values. Readings distinguish the actual finite change from the prediction using starting derivatives. The chain example can have a negative local response but a positive finite change when the move crosses the minimum; its narrative does not infer finite direction from derivative sign. Analogies do not replace mathematical domain restrictions: negative accumulation explicitly means reversed orientation, not a negative physical stock, and zero slope at a saddle is not described as a minimum.

Additional verification covers language/scene invariance, all guided example controls, live chain explanations at zero and across the minimum, equal log-ratio distances, base changes, isolated partial effects, saddle versus bowl classification, and negative oriented accumulation. Browser checks cover the zero-input chain example and mode switch, equal-ratio log examples, and mobile presentation. These are implementation and mathematical checks, not a study of learning outcomes.
