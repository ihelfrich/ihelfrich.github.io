...
- UI interaction checks run in happy-dom. The original suite was published to GitHub Pages. Schema 3 adds narrow-viewport browser checks of the matching stock-flow view and parameter panel, signed debt contributions, and synchronized AK chart/equation selection; this is representative coverage, not exhaustive visual QA of all parameter combinations.
...
## Model-specific companion views (schema 3)

Classification: synthetic, deterministic transformations and counterfactuals of the already specified models. These views read the selected parameter snapshot and period/state without mutating solved results. Canonical plots remain available. Every companion states its question, comparison, units and assumptions; diagrams are chosen for the mechanism rather than forced into one template.

Solow compares consumption per effective worker under a saving change of 10 percentage points (up when s≤.5, down otherwise), with the same initial stocks. AK compares capital and capital per worker as indexes starting at 100, and reports the break-even saving share (n+δ)/a. Trap drift uses the actual upper-envelope technology and distinguishes its unstable equilibrium from its adoption cost. OLG shows one cohort’s wage allocation and next-period retirement consumption, never adding across dates as if they were current aggregate output. IS–LM decomposes the output effect of a ±10-unit G experiment into ΔG+ΔC+ΔI. The direction changes at the upper control limit.

NK shows the moduli of the two homogeneous expectational eigenvalues against the policy coefficient, computed independently of the shock realization; the unit-circle line concerns uniqueness, not shock size. Debt separates rb/(1+g), −gb/(1+g), and −p; their sum is Δb. Optimal growth evaluates the feasible finite-grid Bellman objective at the selected current state and marks its maximizing next-period capital; values are objective minus the computed current-state value. This is a choice landscape, not a simulated time path.

Ramsey compares its consumption path with an exogenous saving rule s*=δk*/y* sharing the same initial stock and analytical steady capital; no finite-horizon welfare ranking is claimed. Two-sector growth compares consumption with an otherwise identical all-production/no-education allocation and labels any first crossing as a horizon-specific result. RBC decomposes log output into log productivity and α times log capital. Open-economy bars compare income and consumption separately at each date; borrowing and asset income reconcile differences. Matching depicts beginning-period employment stocks and the two exclusive transitions per 100 workers. Household asset drift uses a′−a, and the borrowing-bound mass is the joint probability of states whose optimal choice is zero, not a numerical MPC or a continuous Euler-multiplier claim. Sequence-space bars decompose dY into dG, JdG, J²dG and the exact remaining higher-order feedback.

All additive decomposition residuals must be below relative 1e−9. Comparisons use original-period units or explicitly declared indexes/log deviations, not mixed stock/flow axes. Nonfinite values and infeasible Bellman choices are excluded explicitly; no silent zero-filling. Zero shocks, zero population growth, no education, absent trap thresholds and equal interest/growth rates receive meaningful labels. Companion calculations remain frame-rate independent. Existing numerical approximation limits continue to apply.
