...
**Tier 1, primitives.** A named palette derived from the colors the site actually uses today, clustered so that every existing color maps to a primitive within a perceptual distance of ΔE ≤ 2 (CIE76 on Lab). Names are descriptive of hue and step, not of use: `--p-ink-900`, `--p-paper-50`, `--p-signal-500`, `--p-teal-600`, `--p-oxide-600`, `--p-gold-300`, `--p-slate-700`, and so on. Target: under 40 primitives. Alpha variants are expressed as `color-mix(in srgb, var(--p-x) N%, transparent)` at use sites, never as new primitives.
