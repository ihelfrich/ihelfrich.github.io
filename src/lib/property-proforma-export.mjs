...
import {normalizeAssumptionSources,taxEvidenceLines} from './property-tax-evidence.mjs';
...
export function scenarioDocument({name,origin,assumptions,property=null,assumptionSources=null}) {
 ...
 const safeProperty=property?Object.fromEntries(['address','parcelId','recordKey','jurisdiction','taxYear','assessmentYear','sourceUrl','retrievedAt','sourceDataEditedAt'].filter(k=>['string','number'].includes(typeof property[k])).map(k=>[k,String(property[k]).slice(0,1000)])):null;
 const safeSources=normalizeAssumptionSources(assumptionSources,safeProperty,assumptions);
 return {schema:'property-proforma-v1',modelVersion:PROFORMA_VERSION,name:String(name||'Property scenario').slice(0,120),origin:origin==='illustrative'?'illustrative':'user assumptions',createdAt:new Date().toISOString(),assumptions:{...assumptions},property:safeProperty,assumptionSources:safeSources};
}
