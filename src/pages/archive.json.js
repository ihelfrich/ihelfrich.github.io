...
export async function GET({ site }) {
  const [allPapers, datasets, essays, tools] = await Promise.all([
    getCollection("research"),
    getCollection("datasets"),
    getCollection("writing", ({ data }) => !data.draft),
    getCollection("projects", (entry) => !isArchivalProject(entry.id)),
  ]);
  const papers = filterDiscoverableResearch(allPapers);
  const base = site ?? new URL("https://ihelfrich.github.io");
  const records = [
    ...papers.map((entry) => ({ type: "paper", title: entry.data.title, year: entry.data.year, status: entry.data.displayStatus, maturity: entry.data.maturity, authors: entry.data.authors, tags: entry.data.tags, url: new URL(`/research/${entry.id}/`, base) })),
