...
export function createPropertyRegionPanel(root,{getCity=()=>null,onSelect=()=>{},onLocate=()=>{},onView=()=>{},onActivitySelect=()=>{},onNavigation=()=>{},onNavigateArea=()=>{},data=propertyRegionData}={}){
 ...
 function renderMap(){
  ...
  if(!activityFocused){floating.append(el('span',`${money(view.domain[0])} — ${money(view.domain[1])}+`),el('small',`${view.level==='areas'?'Area means':'Individual records'} · gray = unknown${view.partial?' · partial data':''}`));const bar=el('div');bar.className='atlas-key-bar';floating.append(bar);}
  else{floating.append(el('small',features.length?'Larger cells = more source records':'Documents appear in the evidence trail'));const list=el('small');list.className='atlas-activity-legend';for(const layer of activityLayers.filter(l=>l.mappedCount>0)){const row=el('span'),swatch=el('i');swatch.style.backgroundColor=activityFeatures.find(f=>f.layerId===layer.id)?.color||'#b4c6d0';swatch.setAttribute('aria-hidden','true');row.append(swatch,doc.createTextNode(`${layer.label||layer.id}: ${number(layer.mappedCount)}`));list.append(row);}if(activityLayers.some(l=>l.id==='water-materials')){list.append(el('span','Cell color = most common material status'));for(const [id,label,color] of [['lead','Lead indicated','#bb5279'],['galvanized-replacement','Galvanized / replacement','#e39759'],['unknown','Unknown','#a0aab4'],['non-lead','Non-lead indicated','#77cbd3']]){const n=activityFeatures.filter(f=>f.layerId==='water-materials').reduce((sum,f)=>sum+(f.kind==='activity-cell'?(f.statusCounts?.[id]||0):f.status===id?1:0),0);if(!n)continue;const row=el('span'),dot=el('i');dot.style.backgroundColor=color;row.append(dot,doc.createTextNode(`${label}: ${number(n)}`));list.append(row);}}floating.append(list);}
 }
