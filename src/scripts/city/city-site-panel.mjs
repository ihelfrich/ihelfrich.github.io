...
export function createSitePanel(root,{getEvidence=()=>null,load=createSiteSourceLoader()}={}) {
 ...
 let evidence=null,measure=null,points=[],samplingPlan=null,pointOnly=false,products=null,fit=null,generation=0,controller=null,disposed=false;
 ...
 function renderTerrain(terrain) {
  ...
  fit=!pointOnly&&datums.size===1&&terrain.sources.every(s=>s.verticalDatum)?fitTerrainPlane(terrain.samples):null;
  const metrics=el('div',undefined,'development-metrics');
  if(pointOnly) {
    metric(metrics,'Elevation at address point',`${number(terrain.samples[0]?.elevationMetres,2)} m`,'Bare-earth DEM sample');body.append(metrics);
    note(body,'One location only. No parcel area, relief, slope, boundary or development footprint is inferred.');
  } else {
 ...
 function setEvidence(next) {
  generation++;controller?.abort();controller=null;evidence=next;measure=null;products=null;fit=null;points=[];samplingPlan=null;pointOnly=false;
  for(const id of ['geometry','plot','terrain','flood'])$(id).replaceChildren();$('load').disabled=true;$('export').disabled=true;
  const parcel=next?.parcels?.parcel,point=next?.point;$('selection').textContent=parcel?.properties?.address||point?.address||'Select a City or County location.';
  if(!parcel){
    if(point&&Number.isFinite(point.longitude)&&Number.isFinite(point.latitude)&&Math.abs(point.longitude)<=180&&Math.abs(point.latitude)<=90) {
      pointOnly=true;points=[{longitude:point.longitude,latitude:point.latitude}];samplingPlan={status:'point-only',points,requestedCount:1,reason:'No verified parcel geometry.'};
      note($('geometry'),'Address / map point only. Parcel geometry is not connected for this selection.');
      note($('geometry'),`${point.latitude.toFixed(5)}°, ${point.longitude.toFixed(5)}° · WGS84`);
      $('status').textContent='Load one terrain elevation and FEMA flood evidence at this point. Parcel dimensions and grade require verified geometry.';$('load').disabled=false;$('export').disabled=false;
    }else $('status').textContent='Select a City or County location to begin.';
    return;
  }
 ...
 async function refresh() {
  if(disposed||(!measure&&!pointOnly))return;const serial=++generation;controller?.abort();controller=new AbortController();products=null;fit=null;$('load').disabled=true;$('export').disabled=true;$('terrain').replaceChildren();$('flood').replaceChildren();drawPlot();$('status').textContent='Loading independent USGS terrain and FEMA point evidence…';
  try {
  ...
  finally {if(!disposed&&serial===generation){$('load').disabled=false;$('export').disabled=!measure&&!pointOnly}}
 }
 function download() {
  if(!measure&&!pointOnly)return;
  const payload={schema:'st-louis-site-dossier-v1',exportedAt:new Date().toISOString(),classification:pointOnly?'preliminary-point-evidence':'preliminary-site-evidence',point:evidence.point,parcel:evidence.parcels?.parcel||null,parcelSource:evidence.parcels?.source||null,geometry:measure,samplingPlan,sampleLocations:points,terrain:products?.terrain||null,terrainAnalysis:fit,flood:products?.flood||null,evidenceGaps:gaps,limitations:['GIS geometry is not a legal survey.','An address point does not establish a parcel boundary or development footprint.','Terrain is a dated DEM sample, not current surveyed grade.','FEMA query covers the selected point, not the full parcel.','No civil design, utility clearance, hydrologic model or zoning entitlement has been established.']};
