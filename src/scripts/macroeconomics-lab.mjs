...
const solve={solow:engine.solow,ak:engine.ak,trap:engine.trap,olg:engine.olg,islm:engine.islm,nk:engine.newKeynesian,debt:engine.debt,optimal:engine.optimalGrowth};
...
function table(rows){if(!rows.length)return '';const keys=Object.keys(rows[0]);return `<details class="macro-data"><summary>Inspect the calculated values · ${rows.length} rows</summary><div class="macro-table-wrap" tabindex="0" role="region" aria-label="Calculated model values"><table><caption>Model variables; units follow the equations and parameter labels.</caption><thead><tr>${keys.map(k=>`<th scope="col">${escape(k)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${keys.map(k=>`<td>${typeof r[k]==='number'?format(r[k],6):escape(r[k])}</td>`).join('')}</tr>`).join('')}</tbody></table></div></details>`;}
function drawChart(host,{title,xLabel,yLabel,series,points=[],zero=true}){
  ...
  const svg=svgEl(doc,'svg',{viewBox:`0 0 ${w} ${h}`,role:'img','aria-label':`${title}. Horizontal axis: ${xLabel}. Vertical axis: ${yLabel}.`});svg.style.height=`${h}px`;fig.append(svg);
  ...
  series.forEach((s,i)=>{
    ...
    svg.append(svgEl(doc,'path',{d:values.map((p,j)=>`${j?'L':'M'}${X(p.x)},${Y(p.y)}`).join(' '),fill:'none',stroke:colors[i%colors.length],'stroke-width':2.3,'stroke-dasharray':s.dash?'6 4':'','vector-effect':'non-scaling-stroke'}));
  });
  ...
  const probe=doc.createElement('output');probe.className='macro-probe';probe.textContent='Point to the graph to inspect values. The complete table is under Computation.';fig.append(probe);
...
function drawFlow(host,row,step,T,onStep,isAK){
  ...
  let top=host.querySelector('[data-step-control]'),body=host.querySelector('[data-flow-body]');
  if(!top){top=doc.createElement('label');top.className='macro-period';top.dataset.stepControl='';top.innerHTML='<span>Inspect period <strong></strong></span><input type="range" min="0" aria-label="Period shown in the flow diagram">';host.append(top);top.querySelector('input').addEventListener('input',e=>onStep(Number(e.target.value)));}
  top.querySelector('strong').textContent=step;top.querySelector('input').max=T;top.querySelector('input').value=step;
  if(!body){body=doc.createElement('div');body.dataset.flowBody='';host.append(body);}host=body;host.replaceChildren();
...
function metrics(state){const {model:id,result:r,params:p}=state;
  switch(id){
...
function interpretation({model:id,result:r,params:p}){
...
function plots(host,state){host.replaceChildren();const {model:id,result:r,params:p}=state;
  const series=(rows,key,mul=1)=>rows.map(v=>({x:v.t,y:v[key]*mul}));
  const path=(title,names,yLabel='Units per worker',xLabel='Period')=>drawChart(host,{title,xLabel,yLabel,series:names.map(([name,key,mul=1])=>({name,values:series(r.rows,key,mul)}))});
  if(id==='solow'||id==='trap'){
    const steady=id==='solow'?[r.steady]:r.roots.map(q=>q.k),initial=id==='solow'?r.rows[0].q:p.k0,max=Math.max(initial*1.3,...steady.filter(x=>x!=null).map(x=>x*1.3),1);
    const qs=engine.grid(0,max),f=q=>id==='solow'?p.B*q**p.alpha:engine.trapOutput(q,p),upkeep=id==='solow'?r.breakEven:p.n+p.delta;
    drawChart(host,{title:'Investment and the cost of keeping up',xLabel:id==='solow'?'Capital / effective worker':'Capital / worker',yLabel:id==='solow'?'Goods / eff. worker / period':'Goods / worker / period',series:[{name:'Investment',values:qs.map(x=>({x,y:p.s*f(x)}))},{name:'Required investment',values:qs.map(x=>({x,y:upkeep*x})),dash:true}],points:steady.filter(x=>x!=null).map(x=>({x,y:upkeep*x}))});
    ...
    const ss=engine.grid(0,.6);drawChart(host,{title:'Saving changes the permanent growth rate',xLabel:'Saving (% of output)',yLabel:'Growth / worker (%)',series:[{name:'Exact per-worker growth',values:ss.map(s=>({x:s*100,y:100*((1+s*p.a-p.delta)/(1+p.n)-1)}))}],points:[{x:p.s*100,y:r.growth*100}]});
  }else if(id==='olg'){
    const ks=engine.grid(0,Math.max(p.k0,r.steady)*1.6);drawChart(host,{title:'One generation builds the next generation’s capital',xLabel:'Capital / young worker today',yLabel:'Capital / young worker next',series:[{name:'Capital transition',values:ks.map(x=>({x,y:r.savingShare*(1-p.alpha)*p.B*x**p.alpha/(1+p.n)}))},{name:'Unchanged capital (45°)',values:ks.map(x=>({x,y:x})),dash:true}],points:[{x:r.steady,y:r.steady}]});path('Capital across generations',[['Capital / young worker','k']],'Units / young worker','Generation');
  }else if(id==='islm'){
    ...
    drawChart(host,{title:'Fiscal comparative statics',xLabel:'Government purchases',yLabel:'Equilibrium output',series:[{name:'Output with money market clearing',values:engine.grid(0,200,61).map(x=>({x,y:engine.islm({...p,G:x}).Y}))}],points:[{x:p.G,y:r.Y}]});
  }else if(id==='nk'){
    ...
    for(const [key,title,yLabel]of [['policy','The optimal capital policy','Next-period capital'],['consumption','The optimal consumption policy','Consumption']])drawChart(host,{title,xLabel:'Current capital',yLabel,series:r.z.map((z,j)=>({name:`Productivity ${format(z)}`,values:r.grid.map((x,i)=>({x,y:r[key][j][i]}))}))});
  }
}
function diagnostic(state){const {model:id,result:r}=state;
...
function renderReading(host,state){const model=byId[state.model];
  ...
  else {const r=state.result,rows=state.model==='optimal'?r.grid.map((k,i)=>({k,lowCapital:r.policy[0][i],highCapital:r.policy[1][i],lowConsumption:r.consumption[0][i],highConsumption:r.consumption[1][i]})):r.rows||[{Y:r.Y,r:r.r,C:r.C,I:r.I}];host.innerHTML=`<p>${escape(model.computation)}</p><p class="macro-diagnostic">${escape(diagnostic(state))}</p>${table(rows)}`;}
...
export function initializeMacro(doc){
  ...
  const state={model,params:{...byId[model].defaults},result:null,mode:'explore',view:['solow','ak'].includes(model)?'flow':'dynamics',step:0};
  ...
  $('[data-reading-nav]').innerHTML=['explore','equations','computation'].map((mode,i)=>`<button type="button" data-mode="${mode}" aria-pressed="${i===0}">${mode[0].toUpperCase()+mode.slice(1)}</button>`).join('');
  ...
  function visual(){
    const host=$('[data-plots]'),flow=$('[data-flow]');
    if(state.view==='flow'&&['solow','ak'].includes(state.model)){host.hidden=true;flow.hidden=false;drawFlow(flow,state.result.rows[state.step],state.step,state.params.T,step=>{state.step=step;visual();},state.model==='ak');}
    else {host.hidden=false;flow.hidden=true;plots(host,state);}
    const views=$('[data-view-nav]');if(views){views.hidden=!['solow','ak'].includes(state.model);views.querySelectorAll('[data-view]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.view===state.view)));}
  }
  function update(){try{const m=byId[state.model];for(const f of m.fields){const x=state.params[f.key];if(f.options){if(!f.options.some(([v])=>String(v)===String(x)))throw Error(`Choose a valid ${f.label}.`);}else if(!Number.isFinite(x)||x<f.min||x>f.max)throw Error(`${f.label} is outside its allowed range.`);}
      state.result=solve[state.model](state.params);$('[data-error]').hidden=true;
      $('[data-metrics]').innerHTML=metrics(state).map(([label,value])=>`<div><span>${escape(label)}</span><strong>${escape(value)}</strong></div>`).join('');
      $('[data-live]').textContent=interpretation(state);visual();renderReading($('[data-reading]'),state);
    }catch(e){$('[data-error]').textContent=e.message;$('[data-error]').hidden=false;}
  }
  root.addEventListener('input',event=>{const input=event.target;if(!input.matches('[data-param]'))return;const key=input.dataset.param,field=byId[state.model].fields.find(f=>f.key===key);state.params[key]=key==='kind'?input.value:Number(input.value);state.step=Math.min(state.step,state.params.T||0);const value=root.querySelector(`[data-value="${key}"]`);if(value){const percent=field.unit?.startsWith('%')||field.unit==='percentage points';value.textContent=(percent?format(100*state.params[key]):format(state.params[key]))+(percent?(field.unit==='percentage points'?' pp':'%'):'');input.setAttribute('aria-valuetext',value.textContent);}update();});
  root.addEventListener('click',event=>{const button=event.target.closest('button');if(!button)return;
    if(button.dataset.model){state.model=button.dataset.model;state.params={...byId[state.model].defaults};state.view=['solow','ak'].includes(state.model)?'flow':'dynamics';state.step=0;root.querySelectorAll('[data-model]').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));try{win.history.replaceState(null,'',`#${state.model}`);}catch{}controls();update();}
    if(button.dataset.mode){state.mode=button.dataset.mode;root.querySelectorAll('[data-mode]').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));renderReading($('[data-reading]'),state);}
    if(button.dataset.view){state.view=button.dataset.view;visual();}
    if(button.hasAttribute('data-reset')){state.params={...byId[state.model].defaults};state.step=0;controls();update();}
  });
  root.addEventListener('change',event=>{if(event.target.matches('[data-model-select]'))root.querySelector(`[data-model="${event.target.value}"]`)?.click();});
  let resizeFrame;const resize=()=>{if(!state.result)return;if(win?.cancelAnimationFrame)win.cancelAnimationFrame(resizeFrame);resizeFrame=win?.requestAnimationFrame(()=>visual());};
  if(win?.ResizeObserver){const ro=new win.ResizeObserver(resize);ro.observe($('[data-plots]'));ro.observe($('[data-flow]'));}
