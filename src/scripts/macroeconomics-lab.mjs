...
function drawChart(host,{title,xLabel,yLabel,series,points=[],zero=true,selectedX,onSelect,selectKey,selectStep=1,selectedSeries}){
  ...
  const probe=doc.createElement('output');probe.className='macro-probe';probe.textContent=onSelect?'Click a point or use the arrow keys.':'Hover for values.';fig.append(probe);
...
function interpretation(state){const {model:id,result:r,params:p}=state;if(extended[id])return extendedInterpretation(state,format,pct);
  ...
  if(id==='trap')return r.roots.length===2?`There are two positive crossings. From ${format(p.k0)}, capital per worker initially ${change>0?'rises':'falls'}. The unstable crossing separates the two long-run regions.`:`This setting has ${r.roots.length} positive crossing${r.roots.length===1?'':'s'}. `;
  ...
  return `The finite-grid solver ${r.converged?'converged':'reached its iteration limit'} after ${r.iterations} iterations. ${r.boundary} of ${2*p.N} policy choices hit a grid edge. Check grid sensitivity before interpreting the policy.`;
}
...
function renderReading(host,state){const model=byId[state.model];
  if(state.mode==='explore')host.innerHTML=`<div class="macro-explore"><div><h3>The mechanism</h3><p>${escape(model.intuition)}</p><p class="macro-try">${escape(model.experiment)}</p></div><div class="macro-question"><h3>Exercise</h3><p>${escape(model.exercise.question)}</p><details><summary>Solution</summary><p>${escape(model.exercise.answer)}</p></details></div></div>`;
  ...
  const source=host.ownerDocument.createElement('p');source.className='macro-source';source.innerHTML=`Model reference: <a href="${escape(model.source.url)}" target="_blank" rel="noopener">${escape(model.source.label)}</a>.`;host.append(source);
}
